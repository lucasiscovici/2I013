...
class goal_one_to_one(AS):
    ...
    def check_attack(self):
        ...
        return self.ma_position.distance(self.ball.position) < self.autre_position.distance(self.ball.position) or self.ball_position.distance(self.ma_position) < 10
    #A REVOIR
    def attack(self):
...
#                    self.trouve()
                ...
                print("jj",self.ma_position, self.autre_position)
                jj=usefull().has_ball_merge(self.state,s,10)
                ...
                if jj:
                    if abs(self.goal2.x- (self.config.position.x)) <=35:
                        ...
                        if self.autre.position.y > self.ma_position.y:
                        ...
                        y=V2D(self.ball.position.x + (abs(self.autre_position.x-self.ma_position.x)/1.3)*self.mon_sens ,yy)-self.state.ball.position
                        print(y)
                        y=(y).norm_max(1.6)
                        return SA(V2D(0,0),y)
                else:
