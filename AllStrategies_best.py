...
class goal_one_to_one(AS):
    ...
    def check_attack(self):
        ...
        return self.ma_position.distance(self.ball.position) < self.autre_position.distance(self.ball.position)
    #A REVOIR
    def attack(self):
...
#                    self.trouve()
                    ...
                    return SA(V2D(0,0),(y).norm_max(2.4))
                else:
                    y=self.goal2-self.state.ball.position
                    return SA(V2D(0,0),(y).norm_max(1.5))
            else:
                jj=usefull().has_ball_merge(self.state,s,5)
                if jj:
                    if abs(self.goal2.x- (self.config.position.x)) <=35:
                        y=self.goal2 -self.state.ball.position
                        return SA(V2D(0,0),y)
                        ...
                        y=V2D(self.ball.position.x + (abs(self.autre_position.x-self.ma_position.x)/1.5)*self.mon_sens ,yy)-self.state.ball.position
                        y=(y).norm_max(1.5)
                        return SA(V2D(0,0),y)
                else:
