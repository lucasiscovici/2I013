...
DEF1=V2D(T2_BUT+1*T1_SENS,GAME_HEIGHT/2)
DEF2=V2D(T1_BUT+1*T2_SENS,GAME_HEIGHT/2)
ZONE_GOAL=0
ANGLE1=0
ANGLE2=math.pi
...
#DEFENSEUR######################################
#############################################
class defenseur_one_to_one(AS):
    def __init__(self,state):
	AS.__init__(self,"defenseur one_to_one")
	self.state=state

    def compute_strategy(self):

	if self.ma_position.distance(self.ball_position)<self.autre_position.distance(self.ball_position):
		return self.to_change(TO_ATTACK)
	if not self.have_ball():
		sgl=self.simulate(self.moi,self.state)
		if self.dvt_lui(sgl.position):
			return self.to_change(TO_GOAL)

		if  self.In_m(self.projector_ball_x(self.ball,self.ma_position.x),self.mes_goal.y,GAME_GOAL_HEIGHT/2):
			return SA(V2D(0,self.projector_ball_x(self.ball,self.ma_position.x)-self.ma_position.y),V2D())

		else:
			return SA(V2D(0,self.d_but().y-self.ma_position.y),V2D())
	
	else:
		return self.degage_balle()

    def __getattr__(self,name):
       	return getattr(self.state,name)         

class Bibli_Defenseur:
    def __init__(self,state):
	self.state=state

    def va_vers_balle(self):
	return SA(self.ball_position-self.ma_position,V2D())

    def degage_balle(self):
	return SA(V2D(),V2D((600)*self.mon_sens,-200))
    
    def retourne_defense(self):
	return SA(self.ma_def-self.ma_position,V2D()) 	
